...
class CleaningAgent(CellAgent):
    ...
    def needs_charging(self):
        ...
        Returns True if battery is below 20% and not at home station.
        """
        return self.battery < 20 and self.cell != self.home_station

    def is_at_station(self):
        """
        Checks if the agent is currently at the charging station.
        """
        return self.cell == self.home_station

    def charge_battery(self):
        """
        Charges the battery by 5% per step when at charging station.
        """
        if self.is_at_station() and self.battery < 100:
    ...
    def find_path_to_station(self):
        """
        Uses BFS to find shortest path to home charging station.
    ...
    def move_to_station(self):
        """
        Moves one step towards the charging station following the calculated path.
    ...
    def move_to_dirty_cell(self):
        ...
        # Buscar celdas vecinas con suciedad
        ...
        if dirty_neighbors.cells:
            # Moverse a una celda sucia
            self.cell = dirty_neighbors.select_random_cell()
            self.battery -= 1
            self.movements += 1
            return True
        else:
            # No hay celdas sucias cerca, moverse aleatoriamente
    ...
    def step(self):
        # Si está en la estación y la batería no está llena, cargar
        if self.is_at_station() and self.battery < 100:
            self.charge_battery()
            return
